...
// import Key from "./Key";
import type {BookingType, ShowMonths, IControls, blockedDaysType } from "./types";

dayjs.extend(isBetween);

interface CalendarProps {
  bookings?: BookingType[];
  ChangeView?: () => any; // <-- type the function
...
const Calendar: React.FC<CalendarProps> = ({
  bookings = [],
  ChangeView = () => <></>, // default empty function returning JSX
  ...
  const [dateswithEvent, setDatesWithEvents] = useState([]);
  ...
  useEffect(() => {
    const { halfDays, bookedDays, dates } = handleBookings({
      bookings,
      year: activeYear,
    });
  ...
  return (
    <>
      <div className="viewChange">{ChangeView()}</div>
      <section
        id="yearTitle"
        className={`calendar ${layoutClassName}`}
